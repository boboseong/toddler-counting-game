...
export default function FeedGame({ level, stars, tapGap, onHome, onWin, onResult }: GameProps) {
  ...
  const handleConfirm = (rid: number) => {
    if (rid !== roundIdRef.current) return;
    ...
    <GameFrame>
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col items-center justify-between overflow-y-auto px-4 pb-4 pt-2 no-scrollbar short:pb-2 short:pt-0">
        ...
        <div className="flex w-full max-w-3xl flex-col items-center gap-2 short:gap-1">
          <div className="flex h-10 items-center text-xl text-slate-500 sm:text-2xl short:text-base">
            {locked && phase === "play" ? (
              <ListenChip />
            ) : phase === "done" ? (
              "배불러요! 🎉"
          ...
          {needConfirm && phase === "play" && !locked ? (
            <motion.button
              ...
              whileTap={{ scale: 0.93 }}
              onPointerDown={() => handleConfirm(round.id)}
